...
const CollectionCreate = () => {
  ...
  const {apiUrl, corsUrl, getNonce} = useApi()
  const {
    ...
    try {
      const tx = await createNFTContract(
        isSingle
          ...
          : await getSongAlbumMultiFactoryContract(),
        name,
        symbol,
        "5000000000000000",
        account
      ...
      console.log('Factory contract connection result is ===================', res)
      res.events.map(evt => {
          ...
          img.onload = function () {
            ...
            clipImage(img, x, y, size, size, async logodata => {
              try {
                ...
                console.log('IPFS data is ', data);
                const resultsavedata = await axios({
                  method: 'post',
                  url: `https://protected-beach-33448.herokuapp.com/${apiUrl}/collectionSongAlbum/collectiondetails`,
                  data: JSON.stringify(data),
                  headers: {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${authToken}`,
                ...
                if(resultsavedata.data.status == "success"){
                  ...
                  showToast('success', 'Congratulation!' , 'Album has been created successfully! ')
                  setCreating(false)
                  // history.push('/create')
                  navigate('/create')
                }
      ...
      <div className={styles.inner}>
        ...
        {!isRegister && (
          <div className={styles.fee}>
            <InfoIcon/>
            &nbsp;0.1 AVAX are charged to create a new .
          </div>
        )}
