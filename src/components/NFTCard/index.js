...
import { useAuctionContract } from '../../contract/auctions';
import useTokens from '../../hooks/useTokens';

import iconPlus from '../assets/plus.svg';
import wFTMLogo from '../assets/wftm.png';
...
const BaseCard = ({ item, loading, style, create, onCreate, onLike }) => {
  const { storageUrl, apiUrl, likeItem, likeBundle } = useApi();
  ...
  const handleChange = async (e, _id) => {
    ...
    const checkedDetailData = {
      check: e.target.checked,
      id: _id
    }
    const checkedSaveData = await axios({
      method:'post',
      url: `${apiUrl}/nftitems/setItemCheck`,
      data: JSON.stringify(checkedDetailData),
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${authToken}`,
      },
    })
    console.log("After Updated New NFT items ", checkedSaveData)
    if(checkedSaveData.data.status == "success"){
      item.checked = checked;
      console.log("Chceckbox updated successfully")
    }
    else {
      console.log("Checked update Error")
    }
  }
        ...
        {create ? (
          ...
          <>
            { account === "0xce31E9F5503FaBdCBd99137944F6f71933831cb9" && (
              <div>
                <Checkbox checked={!checked?item.checked:checked} onChange={(e) => handleChange(e, item._id)} color="secondary"/>
                <Link
                  to={
                    item.items
                      ? `/bundle/${item._id}`
                      : `/marketplace/${item.contractAddress}/${item.tokenID}/${item.owner}`
                  }
            ...
            { (account !== "0xce31E9F5503FaBdCBd99137944F6f71933831cb9" && (!item.checked || item.checked === false)) ? (
              ...
              <div>This NFT was hided by admin</div>
            )
            }
